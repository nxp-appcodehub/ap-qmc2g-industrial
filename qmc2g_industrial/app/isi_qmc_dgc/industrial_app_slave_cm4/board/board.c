...
void BOARD_ConfigMPU(void)
{
    ...
    /* extend non-cacheable region by secwd_init_data section
     * hence, they must be immediately after each other
     */
    assert((nonCacheStart + nonCacheSize) == secwdInitDataStart);
    nonCacheSize += secwdInitDataSize;
    ...
    /* Region 1 setting: Memory with device type, not shareable, non-cacheable - consider OCRAM ECC1 - RPC msg (required by MAD team) */
    MPU->RBAR = ARM_MPU_RBAR(1, 0x20340000U);
