 ...
 * successful call to this function using the LWDGU_InitWatchdog() function.
 ...
 * Specifies after how many ms the grace watchdog should expire (if it is running).
 ...
 * Do not forget to also initialize the single watchdogs using LWDGU_InitWatchdog()!
 */
 ...
 * which are registered inside the unit will be ticked. If any logical watchdog expires, 
 * then the grace watchdog is started. The ID of the expired
 ...
 * @retval kStatus_LWDG_TickInvArg
 ...
 *   start
 *   :ret = kStatus_LWDG_Err;
 *   if () then (pUnit == NULL || pIsRunning == NULL || id >= pUnit->lwdgsCount)
 *     :ret = kStatus_LWDG_InvArg;
 *   else (else)
 ...
 *   start
 *   :ret = kStatus_LWDG_Err;
 *   if () then (pUnit == NULL || pRemainingTicks == NULL || id >= pUnit->lwdgsCount)
 *     :ret = kStatus_LWDG_InvArg;
 *   else (else)
