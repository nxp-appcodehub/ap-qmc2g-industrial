...
const char *classend (const char *p) {
  switch (*p++) {
    case L_ESC: {
      ...
      if (*p == '\0')
      {
        LWIP_PLATFORM_DIAG(("malformed pattern (ends with '%0')"));
      }
#endif
      return p+1;
    }
    case '[': {
      ...
      do {  /* look for a `]' */
        ...
        if (*p == '\0')
        {
          LWIP_PLATFORM_DIAG(("malformed pattern (missing ])"));
        }
#endif
        //coverity[overflow]
        if (*(p++) == L_ESC && *p != '\0')
...
const char *matchbalance (MatchState *ms, const char *s,
  ...
  if (*p == 0 || *(p+1) == 0)
  {
    LWIP_PLATFORM_DIAG(("unbalanced pattern"));
  }
#endif
  if (*s != *p) return NULL;
  else {
    ...
    case L_ESC: {
      switch (*(p+1)) {
        ...
        case 'f': {  /* frontier? */
          ...
          if (*p != '[')
          {
            LWIP_PLATFORM_DIAG(("missing '[' after \\\\f in pattern"));
          }
#endif
...
const char *pattern_pmatch(const char *p, const char *pe, const char *pat)
{
    MatchState ms = {.src_init = p, .src_end = pe};
    return match(&ms, p, pat);
}


const char *pattern_match(const char *str, size_t len, const char *pat)
{
    MatchState ms = {.src_init = str, .src_end = str + len};
    return match(&ms, str, pat);
}
