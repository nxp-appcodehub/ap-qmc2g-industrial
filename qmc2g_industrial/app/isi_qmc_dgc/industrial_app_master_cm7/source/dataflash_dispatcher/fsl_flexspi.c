...
void FLEXSPI_GetDefaultConfig(flexspi_config_t *config)
{
    ...
    for (uint8_t i = ((uint8_t)FSL_FEATURE_FLEXSPI_AHB_BUFFER_COUNT - 2U); i < (uint8_t)FSL_FEATURE_FLEXSPI_AHB_BUFFER_COUNT; i++)
    {
        config->ahbConfig.buffer[i].enablePrefetch = true; /* Default enable AHB prefetch. */
        config->ahbConfig.buffer[i].masterIndex = 0x0U;
...
void FLEXSPI_SetFlashConfig(FLEXSPI_Type *base, flexspi_device_config_t *config, flexspi_port_t port)
{
	...
	//if (port == kFLEXSPI_PortA1)
	{
	    ...
	    uint32_t rxSampleClock = (base->MCR0 & FLEXSPI_MCR0_RXCLKSRC_MASK) >> FLEXSPI_MCR0_RXCLKSRC_SHIFT;
		base->FLSHCR4 |= (FLEXSPI_FLSHCR4_WMOPT1_MASK|FLEXSPI_FLSHCR4_WMOPT2_MASK);
		...
		if (config->enableWriteMask)
		{
			...
			base->FLSHCR4 &= ~FLEXSPI_FLSHCR4_WMOPT2_MASK;
			if( index == 0)
			{
				base->FLSHCR4 |= FLEXSPI_FLSHCR4_WMENA(1);
			}
			else
			{
				base->FLSHCR4 |= FLEXSPI_FLSHCR4_WMENB(1);
			}
		}
#else
		/* Configure write mask. */
...
status_t FLEXSPI_WriteBlocking(FLEXSPI_Type *base, uint32_t *buffer, size_t size)
{
    ...
    while (0U != size)
    {
        ...
        /* If this is the first time data has been loaded into the TxFIFO,
         * then we need to trigger the start of the IPCMD, but we only
        ...
        if (not_first_data == 0)
        {
        	...
        	/* Set not_first_data so IPCMD will not be triggered again */
        	not_first_data = 1;
        }
...
status_t FLEXSPI_TransferBlocking(FLEXSPI_Type *base, flexspi_transfer_t *xfer)
{
    ...
    else if (xfer->cmdType == kFLEXSPI_Command)
    {
        base->IPCMD |= FLEXSPI_IPCMD_TRG_MASK;
    }
    else
