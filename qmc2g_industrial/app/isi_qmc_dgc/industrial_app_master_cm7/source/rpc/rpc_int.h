...
/* currrently lowest possible interrupt priority
 * if RPC timeouts occur, increase priority or timeouts */
#define RPC_INTERCORE_IRQ_PRIORITY                                \
    (configLIBRARY_LOWEST_INTERRUPT_PRIORITY) /*!< priority for the inter-core communication interrupt \
...
#define RPC_SECWD_TIMEOUT_TICKS pdMS_TO_TICKS(20000U)  /*!< timeout in ms for the secure watchdog RPC processing */
#endif
...
/* To fullfil timestamp granularity requirements for RPC_GetTimeFromRTC (10ms) this timeout should be < 10ms! */
...
#define RPC_MCU_TEMP_TIMEOUT_TICKS pdMS_TO_TICKS(5U) /*!< timeout in ms for the MCU Temperature RPC processing */
...
typedef struct
{
    ...
    const uint32_t completionEvent;         /*!< Event flag for notifying about completion of this remote call. */
